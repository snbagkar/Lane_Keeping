...
def camera_callback(data):
    ...
    slide = cv2.resize(imgSliding, (300,300))
    cv2.imshow('SlidingWindow',slide)
    ...
    canny = cv2.resize(imgFinal, (300,300))
    cv2.imshow('Canny',canny)
   # cv2.moveWindow('canny',700,0)

    #cv2.imshow('color',imgColor)
    col = cv2.resize(imgColor, (300,300))
    cv2.imshow('trace',col)
    #cv2.moveWindow('color',1400,0)

    #cv2.imshow('combined',imgThres)
    thres = cv2.resize(imgThres, (300,300))
    cv2.imshow('Combined',thres)
    #cv2.moveWindow('combined',0,500)

    #cv2.imshow('wrap',wrap_output)
    wrap = cv2.resize(wrap_output, (300,300))
